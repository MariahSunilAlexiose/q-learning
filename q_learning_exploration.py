...
class GridWorld:
    ...
    def q_learning_exploration(self, screen):
        for episode in range(self.episodes):
            ...
            for i in range(self.size):
                for j in range(self.size):
                    if np.random.rand() < self.k:
                        ...
                        # Calculate exploration values with bonuses and penalties
                        bonus = self.k / (np.sqrt(1 + self.visit_count[i, j, :]))
                        penalty = -self.visit_count[i, j, :] / (self.visit_count[i, j, :].max() + 1)
                        exploration_values = self.q_table[i, j] + bonus + penalty
            ...
            self.k = max(self.min_epsilon, self.k * self.epsilon_decay)
...
def main():
    ...
    while running:
        ...
        world.q_learning_exploration(screen)
